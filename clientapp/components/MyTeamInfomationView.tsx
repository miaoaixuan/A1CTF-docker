...
import { Users, Trophy, Hash, Copy, Crown, UserCheck, UserMinus, UserPlus, Settings, Trash2 } from "lucide-react";
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
const MyTeamInfomationView: React.FC<MyTeamInfomationViewProps> = ({
    ...
    const fetchScoreBoardData = () => {
        api.user.userGetGameScoreboard(gameid)
            .then((response) => {
                ...
                // 找到当前用户的战队数据
                if (gameInfo?.team_info?.team_id) {
                    const myTeam = response.data.data?.teams?.find(
                        team => team.team_id === gameInfo.team_info?.team_id
                    );
                    setCurrentUserTeam(myTeam);
                }
            })
            .catch((error) => {
    ...
    useEffect(() => {
        if (gameInfo?.team_info?.team_id && scoreBoardData) {
            const myTeam = scoreBoardData.teams?.find(
                team => team.team_id === gameInfo.team_info?.team_id
            );
            setCurrentUserTeam(myTeam);
        }
                    ...
                    <Card>
                        <CardContent className="pt-6">
                            <div className="text-center">
                                ...
                                <p className="text-muted-foreground">请先创建或加入一个战队来查看战队信息</p>
                            </div>
                        </CardContent>
                    </Card>
                </div>
