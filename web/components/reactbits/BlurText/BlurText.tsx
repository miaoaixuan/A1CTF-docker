...
const BlurText: React.FC<BlurTextProps> = ({
  ...
  const springs = useSprings(
    ...
    elements.map((_, i) => ({
      ...
      delay: finished ? 0 : i * delay,
      config: { duration: finished ? 0 : 1000 , easing: easings.easeInOutCubic },
    }))
  );
    ...
    <p ref={ref} className={`blur-text ${className} flex flex-wrap`}>
      {springs.map((props, index) => (
        <animated.span
          key={index}
          style={props as any}
          className="inline-block transition-transform duration-0 will-change-[transform,filter,opacity]"
        >
